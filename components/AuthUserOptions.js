import { Button, Flex, Popover, PopoverArrow, PopoverBody, PopoverContent, PopoverTrigger, Portal } from "@chakra-ui/react";
                ...
                <Portal>
                    <PopoverContent boxShadow="none !important" width="max-content" >
                        ...
                        <PopoverBody textTransform="capitalize" paddingInline={0}>
                            {
                                session?.user.id === userId && authOptions.map(e => <NextChakraLink
                                    key={nanoid()}
                                    display="block" _hover={{ backgroundColor: "gray.100" }}
                                    paddingInline={3} href={e.url}
                                >
                                    {e.name}
                                </NextChakraLink>)
                            }
